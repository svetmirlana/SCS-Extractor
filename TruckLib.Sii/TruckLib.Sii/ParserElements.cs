...
namespace TruckLib.Sii
{
    internal static class ParserElements
    {
        ...
        internal static readonly Parser<IEnumerable<char>> StringDelimiter =
            Parse.Char(StringDelimiterChar).Once();
        ...
        internal static readonly Parser<dynamic> NotDelimitedString =
            from s in Parse.AnyChar.Except(Parse.LineTerminator).Except(OpenCurly).Many().Text()
            select (dynamic)s;

        internal static readonly Parser<UnitHeader> MatUnitHeader =
            from c in ClassName
            from _ in Colon
            from u in DelimitedString.Or(NotDelimitedString)
            select new UnitHeader(c, u);
